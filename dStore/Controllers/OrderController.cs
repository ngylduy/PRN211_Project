    ...
    public class OrderController : Controller
    {
        ...
        private void ExportToExcel(DataTable orders)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                {
                    currentRow++;
                    worksheet.Cell(currentRow, 1).Value = (XLCellValue)orders.Rows[i]["OrderID"];
                    worksheet.Cell(currentRow, 2).Value = (XLCellValue)orders.Rows[i]["MemberName"];
                    worksheet.Cell(currentRow, 3).Value = (XLCellValue)orders.Rows[i]["OrderDate"];
                    worksheet.Cell(currentRow, 4).Value = (XLCellValue)orders.Rows[i]["OrderTotal"];
                }
